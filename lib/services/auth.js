...
class AuthService extends Schmervice.Service {
    ...
    initialize() {
        ...
        this.PallieService = this.server.services().pallieService;
    ...
    */
    async register(user, txn) {
        ...
        try {
            return await this.PallieService.create(user, txn);
        }
        catch (err) {
    ...
    */
    async login(username, password) {
        ...
        const { pallieService, tokenService } = this.server.services();
    ...
    */
    async logout(credentials) {
        ...
        const user = await this.PallieService.getById(credentials.id);
    ...
    */
    async verifyPassword({ username }, password) {
        ...
        const user = await this.PallieService.getByUsername(username);
    ...
    * @returns {object} A token object with a access and refresh tokens, and the token type
    */
    async reauthorize(token, txn) {

        if (!token) {
            throw Boom.badRequest('No token passed for reauthorization');
        }

        return await this.TokenService.validateRefreshToken(token, txn);
    }
    ...
    */
    async invite(user, force, txn) {
        ...
        const { username } = user;
        const existingUser = await this.PallieService.getByUsername(username, txn);

        if (existingUser && force) {
            // For use with "Resend invite"
            await this.PallieService.remove(existingUser.id, txn);
        }
    ...
    */
    async forgotPassword(username, txn) {
        ...
        const user = await this.PallieService.getByUsername(username, txn);
    ...
    */
    async resetPassword(credentials, oldPassword, userHash, forgotPasswordToken, newPassword, txn) {
        ...
        const user = await this.PallieService.getById(hashids.decode(userHash)[0]);
