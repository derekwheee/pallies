...
class TokenService extends Schmervice.Service {
    ...
    */
    async validateRefreshToken(token, txn, tokenArray, errors = []) {

        if (Array.isArray(token)) {
            const [nextToken, ...restTokens] = token;
            return this.validateRefreshToken(nextToken, txn, restTokens);
        }
        ...
        try {
            ...
            // If there are more tokens available, try them
            // Otherwise throw all of the collected errors
            if (tokenArray?.length) {
                const [nextToken, ...restTokens] = tokenArray;
                return this.validateRefreshToken(nextToken, txn, restTokens, [...errors, err]);
            } else {
                throw [...errors, err];
            }
        }
